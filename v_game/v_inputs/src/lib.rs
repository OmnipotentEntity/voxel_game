...
impl InputSystem {
    pub fn new(
        ...
        InputSystem {
            window_input_queue,
            hardware_input_queue,
        }
    }
}

impl<'a> System<'a> for InputSystem {
    type SystemData = (Write<'a, Inputs>, Write<'a, CursorState>);
                ...
                match input {
                    DeviceEvent::Key(key) => {
                        match inputs.keys.entry(KeyCode::from_key(key.scancode)) {
                            Entry::Occupied(mut e) => {
                                e.insert(if key.state == ElementState::Pressed {
                                    KeyState::Pressed
                                } else {
                                    KeyState::Unpressed
                                });
                    ...
                    DeviceEvent::Button { button, state } => {
                        match inputs.mouse_buttons.entry(ButtonCode::from_key(button)) {
                            Entry::Occupied(mut e) => {
                                e.insert(if state == ElementState::Pressed {
                                    KeyState::Pressed
                                } else {
                                    KeyState::Unpressed
                                });
...
impl<'a> System<'a> for CursorLockSystem {
    type SystemData = (Read<'a, CursorState>, Write<'a, WindowDisplay>);
    ...
    fn run(&mut self, (cursor_state, window_display): Self::SystemData) {
        ...
        window_display
            .gl_window()
            .window()
            .set_cursor_visible(cursor_state.visible);
    }
}
