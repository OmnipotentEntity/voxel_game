...
use v_inputs::{ButtonCode, Inputs, KeyCode, KeyState};
use v_transform::*;
use v_voxels::*;
...
impl<'a> System<'a> for PlayerMovement {
    ...
    fn run(
        &mut self,
        (inputs, players, mut positions, mut rotations, mut chunks): Self::SystemData,
    ) {
        for (player, position, rotation) in (&players, &mut positions, &mut rotations).join() {
            let mut delta = (*inputs.get_mouse_delta()).clone();
            delta /= 1000.0;
            delta[0] = if delta[0].abs() > 0.00001 {
                delta[0]
            } else {
                0.0
            };
            delta[1] = if delta[1].abs() > 0.00001 {
                delta[1]
            } else {
                0.0
            };
            ...
            let y: f32 = if *inputs.get_key(&KeyCode::W) == KeyState::Down {
                -0.025
            } else {
                0.0
            } + if *inputs.get_key(&KeyCode::S) == KeyState::Down {
                0.025
            } else {
                0.0
            };

            let x: f32 = if *inputs.get_key(&KeyCode::D) == KeyState::Down {
                0.025
            } else {
                0.0
            } + if *inputs.get_key(&KeyCode::A) == KeyState::Down {
                -0.025
            } else {
                0.0
            };
