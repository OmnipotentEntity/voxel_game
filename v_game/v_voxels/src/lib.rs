...
impl Chunk {
    pub fn new() -> Self {
        let rle: RLE<BlockType> =
            RLE::from(std::iter::repeat(BlockType::Air).cycle().take(CHUNK_SIZE_3));
        Chunk { blocks: rle }
    }
...
impl ChunkStorage {
    ...
    fn world_point_to_chunk_block(place: &Vector3<f32>) -> (Vector3<i32>, Vector3<usize>) {
        ...
        let block_coord = ((place - chunk_coord_f32 * CHUNK_SIZE as f32 * BLOCK_SIZE) / BLOCK_SIZE)
            .map(|x| x as usize);
    ...
    pub fn set_block(&mut self, block: &BlockType, place: &Vector3<f32>) {
        ...
        if block_coord[0] == 0 {
            self.changed_chunks
                .insert(chunk_coord + Vector3::new(-1, 0, 0));
        ...
        if block_coord[1] == 0 {
            self.changed_chunks
                .insert(chunk_coord + Vector3::new(0, -1, 0));
        ...
        if block_coord[2] == 0 {
            self.changed_chunks
                .insert(chunk_coord + Vector3::new(0, 0, -1));
...
impl ChunkMesherSystem {
    pub fn mesh_chunk(
        chunks: &ChunkStorage,
        marker: &mut ChunkMarker,
    ) -> (Vec<VoxelVertex>, Vec<u32>) {
        let dimension = (0..CHUNK_SIZE).into_par_iter();

        let (sender, receiver): (
            Sender<Vec<VoxelVertex>>,
            std::sync::mpsc::Receiver<Vec<VoxelVertex>>,
        ) = channel();
        ...
        dimension.for_each_with(sender, |sender, (block_x)| {
            let mut verts = vec![];

            for block_y in 0..CHUNK_SIZE {
                for block_z in 0..CHUNK_SIZE {
                    let block_coord = Vector3::new(block_x as f32, block_y as f32, block_z as f32)
                        * BLOCK_SIZE
                    ...
                    let top_block =
                        chunks.get_block(&(block_coord + Vector3::new(0.0, 1.0, 0.0) * BLOCK_SIZE));
                    ...
                    let bottom_block = chunks
                        .get_block(&(block_coord + Vector3::new(0.0, -1.0, 0.0) * BLOCK_SIZE));
                    ...
                    let right_block =
                        chunks.get_block(&(block_coord + Vector3::new(1.0, 0.0, 0.0) * BLOCK_SIZE));
                    ...
                    let left_block = chunks
                        .get_block(&(block_coord + Vector3::new(-1.0, 0.0, 0.0) * BLOCK_SIZE));
                    ...
                    let front_block =
                        chunks.get_block(&(block_coord + Vector3::new(0.0, 0.0, 1.0) * BLOCK_SIZE));
                    ...
                    let back_block = chunks
                        .get_block(&(block_coord + Vector3::new(0.0, 0.0, -1.0) * BLOCK_SIZE));
...
impl<'a> System<'a> for ChunkMesherSystem {
    ...
    fn run(&mut self, (entities, chunks, display, mut markers, mut renderers): Self::SystemData) {
        ...
        (&mut markers, &entities)
            .par_join()
            .for_each_with(send, |sender, (marker, entity)| {
                if !marker.changed || !marker.renderable {
                    return;
                }
...
impl<'a> System<'a> for NewChunkPlacementSystem {
    ...
    fn run(
        &mut self,
        (mut chunks, entities, mut chunk_markers, mut positions, mut transforms, lazy): Self::SystemData,
    ) {
